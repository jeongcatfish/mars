...
void draw()
{
  ...
  for (int i=0; i<1; i++) // struggle with detect more than one person..
  {
    if(state == false)  pick[i].print_footprint_right(); // print foot when the pixel is changed.
    else if(state== true) pick[i].print_footprint_left();
    pick[i].remove_right();
    pick[i].remove_left();
  }
  //draw_rect();
...
void human()
{
  ...
  noStroke();
  fill(255, 0, 0);
  ellipse(mouseX, mouseY, skip+7, skip+7);
  /*
  fill(255,255,255);
